  ...
  const performAICheck = async (text) => {
    ...
    try {
      ...
      const prompt = `
        ...
        生徒の「今回の発言」を、以下の【指導方針】および【判定フロー】に従って厳密に審査してください。

        ## 【指導方針（フィードバック生成の前提）】
        子どもがSNSで発信するときには、以下の発達段階に応じた課題を踏まえた指導が求められます。フィードバック生成時はこれらを意識してください。

        * **小学校高学年**: 仲間内の同調や閉鎖的な集団が生まれやすく、言葉が自己肯定感を大きく揺らします。他者を尊重し、場に合った伝え方を促す支援を行ってください。
        * **中学生**: 自意識が強まり、皮肉や感情的反応が起こりやすくなります。意図が正しく伝わる言い方や、公正な視点をもった表現を促す指導が必要です。
        * **高校生**: 社会参加を見据え、仲間内だけでなく公共性や責任を意識した言葉選びを学ぶことが求められます。

        **重要**: SNSでのフィードバックは、単なる攻撃性の抑止だけでなく、他者への想像力、自己表現の調整力、公共性の理解を育む「教育的役割」を果たしてください。
        ...
        * **残虐な表現**
        ...
        言葉自体が丁寧でも、意味合いとして攻撃的、または**「攻撃的と受け取られるリスクがある」**場合はNG (**isAggressive: true**) です。
        ...
        * **冷淡な突き放し・無視**
        ...
        ステップ1, 2で問題がなかった場合のみ、会話の流れに沿っているか判断し、**contextMatchScore** に反映してください。
        ...
        * **isAggressive: true の場合**:
            * **aggressiveFeedback**: なぜその言葉がいけないのか、小学生にも分かるように優しく、しかし毅然と諭してください。「相手がどう思うか」や【指導方針】の観点を理由に挙げてください。
            * **contextMatchScore**: 0 (固定)
            * **contextFeedback**: 空文字列 ("")

        * **isAggressive: false の場合**:
            * **aggressiveFeedback**: 空文字列 ("")
            * **contextMatchScore**: 0〜100で採点 (会話の流れに沿っていれば高く、無関係なら低く)
            * **contextFeedback**: 会話をより良くするためのアドバイスや、肯定的なコメントを入れてください。
        ...
        ## 出力フォーマット (JSONのみ)
        回答は**必ずJSON形式のみ**で出力してください。
        Markdownのコードブロック（\`\`\`json ... \`\`\`）や、挨拶などの余計な文字列は一切含めないでください。
